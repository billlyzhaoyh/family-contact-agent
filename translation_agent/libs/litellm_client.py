...
class LiteLLMClient:
    ...
    def __init__(
        ...
        if not LiteLLMConfig.validate_provider_config(self.provider):
            self.logger.warning(
                f"Provider {self.provider} configuration not found. Available providers: {LiteLLMConfig.list_available_providers()}"
            )
    ...
    def invoke_model(
        ...
        **kwargs,
        ...
        try:
            ...
            # Make the API call
            response = completion(model=model.value, messages=messages, **config)
    ...
    async def invoke_model_async(
        ...
        **kwargs,
        ...
        try:
            ...
            response = await acompletion(model=model.value, messages=messages, **config)
    ...
    def get_model_info(self, model: LiteLLMModel) -> Dict[str, Any]:
        ...
        return {"model": model.value, "provider": provider, "config": config}
